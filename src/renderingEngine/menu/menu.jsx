  ...
  return (
    ...
    >
      <div className="flex-1 basis-[80%] bg-[#e1e2e6] border-b-2 border-[#b3e3fa] flex flex-col items-center justify-center">
        <div
          className="grid grid-cols-4 grid-rows-3 gap-7"
          style={{
            width: "fit-content",
            height: "80%",
            margin: "0 auto",
        ...
        >
          {[...Array(12)].map((_, i) => {
            const ch = channels[i];
            return ch ? (
              <WiiChannel key={ch.id} icon={ch.icon} name={ch.name} hoverFX mode={mode} />
            ) : (
              <WiiChannel key={`placeholder-${i}`} icon="" name="" mode={mode} />
            );
          })}
        </div>
      </div>
      ...
      <WiiMenuFooter>
        <WiiMenuFooterButton icon="/Favicon.png" clickID="settings" mode={mode} />
        <WiiClock />
