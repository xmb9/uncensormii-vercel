...
import { EnterKeyIcon_svg } from '../assets/enter';
...
export function Setup() {
    ...
    useEffect(() => {
        const handleKey = (e) => {
            if ((e.key === 'Enter' || e.key === ' ') && phase === 'initial' && !isFading) {
    ...
    useEffect(() => {
        if (phase === 'initial') {
            document.body.style.backgroundImage = "url('/assets/setup/PressEnterToContinue.png')";
            localStorage.setItem('oobeStage', 'initial');
        }

        if (phase === 'sysBkndSplash') {
            localStorage.setItem('oobeStage', 'sysBkndSplash');
            document.body.style.backgroundImage = "url('/assets/nintendo/NoA_whiteBackgroundLines.png')";
    ...
    // our text arrays that'll be rendered below
    const texts = {
        initial: [
            ...
            >
                <div class="relative w-full h-full flex items-center pl-46 space-x-6">
                    ...
                    <div
                        ...
                        style={{ opacity: phase === 'sysBkndSplash' && !isFading ? 1 : 0 }}
                    >
